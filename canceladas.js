...
function createMLChart(canvasId, country) {
        ...
        Promise.all(
          ...
          .then((datos) => {
            ...
            const dataset = datos.map(
              (objeto) =>
                objeto.data.filter(
                  (item) => item.status.toLowerCase() === "cancelada" && item.pais.toLowerCase() === country.toLowerCase()
                ).length
            );
  ...
  createMLChart("canceladasargentina", "Argentina");
  createMLChart("canceladasbrasil", "Brasil");
  createMLChart("canceladascolombia", "Colombia");
  createMLChart("canceladasmexico", "Mexico");
  createMLChart("canceladasuruguay", "Uruguay");